...
# python3 github-dev-ArteDial.py
...
in_path = f'/media/AllBlue/LanguageData/CLEAN/Bavarian'
out_path = f'/media/AllBlue/LanguageData/PREP/2024SchuMATh-barL-Sock-deuL-DBLI-0001'
temp_path = f'/media/AllBlue/LanguageData/PREP/2024SchuMATh-barL-Sock-deuL-DBLI-0001/Temp'
dir_maker(temp_path)
log_info = []
...
#src_trg = []
#test_trg = []
with open(f'{in_path}/2023ArteDial-barL-BiTe-deuL-0003.csv', 'r') as csv_file:
...
log_info.append(f'Length of Bavarian TEST bar_0003: {len(test_src_trg)}')
#log_info.append(f'Length of Bavarian TEST bar_0003: {len(test_src)}')
#log_info.append(f'Length of German TEST bar_0003: {len(test_trg)}')
with open(f'{temp_path}/test.bar', 'w') as src_file:
    with open(f'{temp_path}/test.deu', 'w') as trg_file:
...
#        txt_file.write(line+'\n')
#with open(f'{temp_path}/test.deu', 'w') as txt_file:
...
#dev_src = []
#dev_trg = []
with open(f'{in_path}/2023ArteDial-barL-BiTe-deuL-0002.csv', 'r') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',',quotechar='"')
    next(csv_reader, None)
    for row in csv_reader:
...
log_info.append(f'Length of Bavarian DEV bar_0002: {len(dev_src_trg)}')
#log_info.append(f'Length of Bavarian DEV bar_0002: {len(dev_src)}')
#log_info.append(f'Length of German DEV bar_0002: {len(dev_trg)}')
...
log_info.append(f'Length of unique Bavarian DEV bar_0002: {len(dev_src_trg_out)}')
...
#log_info.append(f'Length of unique Bavarian text bar_0002: {len(dev_src)}')
#log_info.append(f'Length of unique German text bar_0002: {len(dev_trg)}')
with open(f'{temp_path}/dev.bar', 'w') as src_file:
    with open(f'{temp_path}/dev.deu', 'w') as trg_file:
        for entry in dev_src_trg_out:
            src_file.write(entry[0]+'\n')
            trg_file.write(entry[1]+'\n')
# with open(f'{temp_path}/dev.bar', 'w') as txt_file:
#     for line in dev_src:
#         txt_file.write(line+'\n')
# with open(f'{temp_path}/dev.deu', 'w') as txt_file:
#     for line in dev_trg:
#         txt_file.write(line+'\n')
...
with open(f'{in_path}/2023ArteDial-barL-BiTe-deuL-0001.csv', 'r') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',',quotechar='"')
    next(csv_reader, None)
    for row in csv_reader:
        # ###############################################################################
        ...
        #train_src.append(row[2]) # 0 is Bavarian title
        #train_trg.append(row[3]) # 1 is German title
log_info.append(f'Length of Bavarian TRAIN bar_0001: {len(train_src_trg)}')
...
#log_info.append(f'Length of Bavarian text bar_0001: {len(train_src)}')
#log_info.append(f'Length of German text bar_0001: {len(train_trg)}')
# Remove duplicates to prevent information/knowledge leakage
train_src_trg_out = []
#train_src_out = []
#train_trg_out = []
...
#         train_trg.remove(train_sent)
log_info.append(f'Length of unique Bavarian text bar_0001: {len(train_src_trg_out)}')
...
#log_info.append(f'Length of unique Bavarian text bar_0001: {len(train_src_out)}')
#log_info.append(f'Length of unique German text bar_0001: {len(train_trg_out)}')
with open(f'{temp_path}/train.bar', 'w') as src_file:
    with open(f'{temp_path}/train.deu', 'w') as trg_file:
        for entry in train_src_trg_out:
            src_file.write(entry[0]+'\n')
            trg_file.write(entry[1]+'\n')

# Store the excluded entries (viewing the data indicated that these contain a lot of "real" Bavarian data- even if not properly aligned)
with open(f'{temp_path}/other.bar', 'w') as src_file:
    with open(f'{temp_path}/other.deu', 'w') as trg_file:
        for entry in other_src_trg:
            src_file.write(entry[0]+'\n')
            trg_file.write(entry[1]+'\n')

# with open(f'{temp_path}/train.bar', 'w') as txt_file:
#     for line in train_src_out:
#         txt_file.write(line+'\n')
# with open(f'{temp_path}/train.deu', 'w') as txt_file:
...
def tokenize_german_texts(input_path = f'/media/AllBlue/LanguageData/PREP/2024SchuMATh-barL-Sock-deuL-DBLI-0001/Temp', out_path = f'/media/AllBlue/LanguageData/PREP/2024SchuMATh-barL-Sock-deuL-DBLI-0001'):
...
src_text = []
with open(f'{out_path}/train.bar', 'r') as src_in:
    src_text = src_in.readlines()
with open(f'{out_path}/train.bar', 'w') as src_out:
    for sentence in src_text[33:]:
        src_out.write(sentence)

trg_text = []
with open(f'{out_path}/train.deu', 'r') as trg_in:
    trg_text = trg_in.readlines()
with open(f'{out_path}/train.deu', 'w') as trg_out:
